...
ctx.addEventListener("message", async (event) => {
  console.log({ event, data: event.data, _buildTextures });
  const result = await _buildTextures(
    ...(event.data as [
      number,
      NoiseSettings,
      Uint8ClampedArray,
...
const _buildTextures = async (
  ...
  for (let i = 0; i < numberOfPixels; i++) {
    const a = i * 4;
    let [r, g, b] = [heightData[a], heightData[a + 1], heightData[a + 2]];
    if (r + g + b === 0) {
      continue;
    }
        ...
        if (layerSettings.hard) {
          noiseValue =
            ...
            4 *
              Math.abs(
                openSimplex(
                  layerSettings.shift + r * roughness,
                  g * roughness,
                  b * roughness
                )
              );
